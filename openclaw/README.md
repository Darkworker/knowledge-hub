...
### 1. OpenClaw 2026.3.7 - ContextEngine 革命性更新（2026.3.9）
...
### 2. OpenClaw 3.12 正式发布（2026.3.14）
OpenClaw 3.12 已正式发布，带来以下重要更新：
- **性能优化**：内部基准测试显示内存开销减少 15%
...
**安全提醒**：鉴于近期发现的多项严重安全漏洞（包括高危的命令注入、缓存投毒、DoS、认证绕过等），强烈建议所有用户立即升级至最新版本（2026.3.11+）并遵循安全部署最佳实践。
